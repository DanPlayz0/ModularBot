class TextCommandContext {
  constructor(options) {
    ...
    this.Discord = this.client.pkg.Discord;
    this.MessageEmbed = this.Discord.MessageEmbed;
    this.Permissions = this.Discord.Permissions.FLAGS;

    this.flags = new this.client.pkg.stringToolkit().parseOptions(this.args).flags;
    this.parser = new this.client.pkg.stringToolkit().parseOptions(this.args);
  ...
  async pagify(embeds, options = {}) {
    ...
    embeds.filter(m => m instanceof this.MessageEmbed).map((embed, i) => embed.setFooter({text: `Requested by ${this.author.username} • Page ${i + 1} of ${pages}${embed?.footer?.text ? `\n${embed.footer.text}` : ''}`}));
...
module.exports = {
  TextCommandContext//, InterCommandContext
}
